...
forvalues r = 1/4 {
	...
	if `r' == 1 {
		...
		local n = trim("`n'")
		global title "`n' patients (0-4 hrs)"
		drop  pf* gcs* urin* ph*
		local vars hr bps temp rr urea cr na wcc
	}
	if `r' == 2 {
		global population 4
		keep if time2icu >= 4 & time2icu < 12
		qui count
		local n: di %9.0fc `=r(N)'
		local n = trim("`n'")
		global title "`n' patients (4-12 hrs)"
		drop  pf* gcs* urin* ph*
		local vars hr bps temp rr urea cr na wcc
	}
	if `r' == 3 {
		global population 12
		keep if time2icu >= 12 & time2icu < 24
		qui count
		local n: di %9.0fc `=r(N)'
		local n = trim("`n'")
		global title "`n' patients (12-24 hrs)"
		drop  pf* gcs* urin* ph*
		local vars hr bps temp rr urea cr na wcc
	}
	if `r' == 4 {
		global population 24
		keep if time2icu >= 24 & !missing(time2icu)
		qui count
		local n: di %9.0fc `=r(N)'
		local n = trim("`n'")
		global title "`n' patients (>24 hrs)"
		drop  pf* gcs* urin* ph*
		local vars hr bps temp rr urea cr na wcc
	}
...
graph combine slope_0 slope_4 slope_12 slope_24, ///
	rows(1) ycommon xsize(16) ysize(8)
graph rename severity_slope_time, replace
graph display severity_slope_time
graph export ../outputs/figures/severity_slope_time.pdf ///
    , name(severity_slope_time) replace
cap log close
