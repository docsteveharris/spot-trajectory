...
- focus just on different definitions of trajectory
- work just with global severity

*/

GenericSetupSteveHarris spot_traj an_model_raw_bkwd_times, logon
...
preserve
// Now define an alternative trajectory
local external_call 1
global traj_x `t'
...
*  ================================================================
clonevar ims_ms_`t't1 = ims_ms1
clonevar ims_ms_`t't2 = ims_ms2
clonevar ims_ms_`t't_traj = ims_ms_traj 

local physiology_vars ims_ms_`t't
...
foreach pvar of local physiology_vars {
	...
	* local pvar ims_c
	...
	if "`pvar'"	== "ims_ms_`t't" {
		...
		// NOTE: 2013-06-22 - you do *not* want to redefine this again
		* cap drop `pvar'_traj
		* gen `pvar'_traj = `pvar'2 - `pvar'1 if time2icu <= 96
	...
	* marginsplot, x(`pvar'_tvector) legend(pos(3))
	...
	marginsplot ///
		...
		byopts(rows(1) title("") ///
			subtitle("(A) Complete cases - `t' hour denominator", position(11) justification(left) )) ///
		recastci(rspike) ///
		plotopts(msymbol(o)) ///
	...
	* marginsplot, x(`pvar'_tvector) legend(pos(3))
	...
	marginsplot ///
		...
		bydimension(`pvar'2_k) ///
		byopts(rows(1) title("") ///
			subtitle("(B) Multiple imputation - `t' hour denominator", position(11) justification(left) )) ///
		recastci(rspike) ///
		plotopts(msymbol(o)) ///
		xtitle("Pre-admission trajectory", margin(medium)) ///
		xlabel(1 "{&darr}{&darr}" 2 "{&darr}" 3 "0" 4 "{&uarr}" 5 "{&uarr}{&uarr}", labsize(medium) ) ///
...
graph combine bkwd_ims_ms_6t_grid bkwd_ims_ms_12t_grid bkwd_ims_ms_24t_grid, ///
	rows(1) title(Trajectory (defined per 6, 12, 24 hours)) xsize(8) ysize(4) ///
	name(bkwd_times_sensitivity, replace)
graph display bkwd_times_sensitivity
graph export ../outputs/figures/bkwd_times_sensitivity.$gext ///
    , name(bkwd_times_sensitivity) replace
!rm ../outputs/figures/bkwd_times_sensitivity.$gext_other
