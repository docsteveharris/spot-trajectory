*  =================================================================
*  = Describe characteristics of variables that are being assessed =
*  =================================================================


// Admission physiology data (1st 24 hours)

GenericSetupSteveHarris spot_traj an_tables_traj_vars, logon
global table_name traj_vars

/*
Repeat x2: once for all, once for mid range?
...
- Trajectory
    ...
    - spark distribution
- missingness
- column to indicate if in ICNARC score
For the following vars 
hr bps rr temp urin pf lac wcc plat na cr urea ph ims_c ims_ms ims_c
*/
...
local norm_vars 
local skew_vars ///
    wcc2 ph2 lac2 rr2 pf2 urea2 cr2 urin2 gcs2 plat2  ///
    temp2 hr2 bps2 na2 ph2 lac2 ims_c2 ims_ms2
local range_vars
local bin_vars
local cat_vars
...
* CHANGED: 2013-05-24 - modified to exploit byloop to swap from ICU measurement to traj
* foreach lvl of local bylevels {
forvalues lvl = 1/2 {
    use `working', clear
    * keep if `byvar' == `lvl'
    // Work with ICU admission value
    if `lvl' == 1 {
        local lvl_label "ICU"
    }
    ...
    if `lvl' == 2 {
        ...
        foreach var of local table_vars {
            local root_var = substr("`var'", 1, length("`var'") - 1)
            local traj_var `root_var'_traj
            // NOTE: 2013-05-24 - HACK TO get TRAJ into ICU variable for this loop
            drop `var'
            clonevar `var' = `traj_var'
        }
        local lvl_label "Trajectory"
    }
    ...
    foreach var of local table_vars {
        ...
        if `check_in_list' > 0 {
            ...
            kdensity `var' if !missing(`var'), gen(kx kd) nograph
            // normalise over the [0,1] scale
            qui su kd
            replace kd = kd / r(max)
            egen kx20 = cut(kx) if !missing(kx), group(`sparks')
            replace kx20 = kx20 + 1
            bys kx20: egen kdmedian = mean(kd) if !missing(kd)
...
while `i' <= `lastrow' {
    ...
    foreach var in vcentral vmin vmax vother {
        ...
        * CHANGED: 2013-05-28 - use stata format for 1st 24hr value
        ...
        replace `var'_fmt = "`formatted'" ///
            ...
            & !missing(`var'[`i']) ///
            & bylevel == 1
        // CHANGED: 2013-05-28 - use standard format for trajectory
        local formatted : di %9.1fc `var'[`i']
        di `formatted'
        replace `var'_fmt = "`formatted'" ///
            if _n == `i' ///
            & !inlist(var_type[`i'],"Binary", "Categorical") ///
            & !missing(`var'[`i']) ///
            & bylevel == 2
        // CHANGED: 2013-05-28 - specific format for pH and platelets
        local formatted : di %9.2fc `var'[`i']
        di `formatted'
        replace `var'_fmt = "`formatted'" ///
            if _n == `i' ///
            & !inlist(var_type[`i'],"Binary", "Categorical") ///
            & !missing(`var'[`i']) ///
            & bylevel == 2 & varname[`i'] == "ph2"
        local formatted : di %9.0fc `var'[`i']
        di `formatted'
        replace `var'_fmt = "`formatted'" ///
            if _n == `i' ///
            & !inlist(var_type[`i'],"Binary", "Categorical") ///
            & !missing(`var'[`i']) ///
            & bylevel == 2 & inlist(varname[`i'], "plat2", "urin2")
...
*     local super_heading1 "`super_heading1' & \multicolumn{3}{c}{`bytext'} "
*     local grp_size "`grp_size' patients"
*     local super_heading2 "`super_heading2' & \multicolumn{3}{c}{`grp_size'} "
*     local ++j
* }
...
* local super_heading1 " `super_heading1' & Missing \\"
* local super_heading2 " `super_heading2' & \\"
local super_heading1 "  & \multicolumn{3}{c}{ICU 1\textsuperscript{st} 24 hour value}  & \multicolumn{3}{c}{Pre-admission trajectory}  & Missing \\ "
* Prepare sub-headings
local sub_heading "Median (IQR)"
* CHANGED: 2013-02-07 - drop parameter from column heading and leave blank
* - if needed then Characteristic is preferred

local sub_heading "& \multicolumn{3}{c}{`sub_heading'} &  \multicolumn{3}{c}{`sub_heading'} & (\%) \\"
local sub_heading_cline "\cmidrule(rl){2-4} \cmidrule(rl){5-7}"

xrewide vcentral_fmt vbracket vmiss sparkspike varname, ///
    i(seq) j(bylevel) ///
    lxjk(nonrowvars)

// Handle the fact that you now have 2 varnames because of the hack above
drop varname2 
rename varname1 varname
// And trajectory will have more missing so use this to represent missingness
drop vmiss1
rename vmiss2 vmiss
...
if `sparklines_on' {
    ...
    local justify X[l8] X[r]X[l3]X[l2]  X[r]X[l3]X[l2] X[r1]
...
*/
di "`nonrowvars'"
listtab tablerowlabel vcentral_fmt1 vbracket1 sparkspike1 vcentral_fmt2 vbracket2 sparkspike2 vmiss_percent  ///
